...
export const signup = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const userData = await hashPasswordIfModified(req.body as User);
...
export const login = catchAsync(
  ...
  `;
    ...
    if (!user || !(await correctPassword(password, user.password as string))) {
...
export const updatePassword = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!user || !user.password) {
      return next(new AppError('User not found or password not set', 404));
    }

    // 2) Check current password
    const isCorrect = await bcrypt.compare(
      req.body.password_current as string,
      user.password,
    );
    ...
    // 5) Send new JWT
    createSendToken({ id: user.id } as User, 200, req, res);
  },
);
